...
import it.polito.wa2.group17.login.model.BadLoginResponse
import it.polito.wa2.group17.login.model.LoginRequest
import it.polito.wa2.group17.login.model.LoginResponse
...
interface SignInAndUserInfo {
    ...
    fun findUser(username: String): UserDetailsDto?
}
...
class SignInAndUserInfoImpl(private val userRepository: UserRepository) : SignInAndUserInfo {
    ...
    override fun findUser(username: String): UserDetailsDto? {
        val user = userRepository.findByUsername(username)
        return if (user.isEmpty) null
        else fromEntity(user.get())
    }
}
